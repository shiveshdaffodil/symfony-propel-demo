...
abstract class BaseComment extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'Acme\\BlogBundle\\Model\\CommentPeer';
	...
	protected function loadContent(PropelPDO $con = null)
	{
		...
		$c->addSelectColumn(CommentPeer::CONTENT);
		try {
	...
	public function setCreatedBy($v)
	{
		...
		if ($this->created_by !== $v) {
			$this->created_by = $v;
			$this->modifiedColumns[] = CommentPeer::CREATED_BY;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = CommentPeer::NUM_HYDRATE_COLUMNS.
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(CommentPeer::POST_ID)) {
			$modifiedColumns[':p' . $index++]  = '`POST_ID`';
		}
		if ($this->isColumnModified(CommentPeer::CONTENT)) {
			$modifiedColumns[':p' . $index++]  = '`CONTENT`';
		}
		if ($this->isColumnModified(CommentPeer::CREATED_BY)) {
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
	{
		...
		$result = array(
			...
			$keys[2] => ($includeLazyLoadColumns) ? $this->getContent() : null,
			$keys[3] => $this->getCreatedBy(),
		);
		if ($includeForeignObjects) {
			if (null !== $this->aPost) {
				$result['Post'] = $this->aPost->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
			}
			if (null !== $this->aUser) {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CommentPeer::CREATED_BY)) $criteria->add(CommentPeer::CREATED_BY, $this->created_by);
	...
	public function getPost(PropelPDO $con = null)
	{
		if ($this->aPost === null && ($this->post_id !== null)) {
			$this->aPost = PostQuery::create()->findPk($this->post_id, $con);
